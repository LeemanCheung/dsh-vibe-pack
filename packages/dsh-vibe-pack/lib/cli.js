...
function manager(opts) {
	return new PackManager(opts.root);
}
function source(value) {
	const normalized = value.toLowerCase();
	return {
		kind: normalized.endsWith(".zip") || normalized.endsWith(".dshpack") ? "archive" : "directory",
		path: value
	};
}
program.option("--root <path>", "managed DSH root", dshHomePath());
program.command("inspect <source>").action(async (s) => {
	const result = await manager(program.opts()).inspect(source(s));
...
program.command("uninstall <id>").option("--force", "remove locally modified owned resources").action(async (id, o) => {
	await manager(program.opts()).uninstall(id, o);
});
...
program.command("export <id>").description("write .dshpack ZIP bytes to stdout").action(async (id) => {
	const encoded = await manager(program.opts()).export(id);
	process.stdout.write(Buffer.from(encoded.slice(18), "base64"));
});
